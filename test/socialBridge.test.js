...
test('extension manifest, configuration and scripts are valid and narrowly scoped', async () => {
  ...
  assert.equal(manifest.version, '1.10.10');
  ...
  assert.match(pageSource, /await api\('wallet\/remark\/list'\)/);
  assert.match(pageSource, /savedNote !== expectedNote/);
...
test('DeBot page bridge polls while hidden, consumes the expected channels and uses the observed API payloads', async () => {
  ...
  assert.equal(personalHeartbeat.payload.version, '1.10.10');
  ...
  assert.equal(recoveredHeartbeat.payload.version, '1.10.10');
