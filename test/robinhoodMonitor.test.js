...
test('DeBot BSC wallet events persist without calling RPC and still enforce the local wallet library', async () => {
  ...
  const rpcClient = {
    async getBlockNumber() { throw new Error('RPC must not be called'); },
    async getLogs() { throw new Error('RPC must not be called'); }
  };
  ...
  const valid = await monitor.ingestExternalWalletEvent({
    ...
    tokenSymbol: 'TEST', tokenName: 'Test Token', rawTokenAmount: '500', tokenDecimals: 2,
    blockTimestamp: 100
  });
  assert.equal(valid.accepted, true);
  assert.equal(store.listMonitorEvents({ limit: 10 })[0].tokenAmount, '5');
