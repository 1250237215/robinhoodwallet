...
It does not read or export DeBot cookies, passwords, local storage, or `sub_token`. A page-world bridge calls DeBot's own authenticated social endpoints, normalizes the returned records, and sends only allowlisted tweet, watchlist, heartbeat, and command-result fields to the Radar social API. Follow, unfollow, rename, avatar, description, and other non-tweet activity is discarded by the page bridge, service worker, and durable outbox. Raw DeBot response objects are never uploaded.
...
Live posts are captured from DeBot's WebSocket immediately, including text and binary browser frames, with an independent two-second REST poll of the monitored-account timeline and the recovery alarm as fallbacks. A primary REST request is abandoned after 3.5 seconds so one stalled response cannot recreate the old 20-second blind spot. Featured and all-account feeds run separately every 15 seconds, so a slow or failed optional feed cannot block the monitored timeline. Returning online, restoring a cached page, focusing the DeBot tab, or making it visible triggers an immediate coalesced primary poll. Each post carries the millisecond `discoveredAt` from its first browser observation; duplicates, feed merges, batches, and upload retries preserve that original value. Any failed primary timeline poll marks the Radar connection as unhealthy until a complete primary poll succeeds. The outbox protects every post the browser has already observed. DeBot's current timeline response is limited to the latest 50 records per feed, so an extended period with Chrome fully closed, the computer asleep, or the DeBot session unavailable can exceed the upstream recovery window; those older, never-observed records cannot be reconstructed by the extension.
